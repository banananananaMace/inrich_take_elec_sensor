...
void SX1280_Init(uint32_t RF_FREQUENCY )
{
    ...
    packetParams.Params.LoRa.HeaderType =LORA_PACKET_VARIABLE_LENGTH;// LORA_PACKET_FIXED_LENGTH;
...
uint8_t  Process_Appstate_0(void)
{
    ...
    case APP_TX_TIMEOUT:  //发送超时
        break;
    case APP_LOWPOWER:
        AppState=10;//下次不执行 只执行一次
        //  SendtoStation_sx1280_frame(REQ,0,0,0);
        //	GpioWrite( POW_OE_PORT, POW_OE_PIN , GPIO_PIN_RESET );
        //SendtoStation_sx1280_frame(MESSAGE,6,Temperature_type,0);
        //	GpioWrite( POW_OE_PORT, POW_OE_PIN , GPIO_PIN_SET );
        Radio.SetDioIrqParams( RxIrqMask, RxIrqMask, IRQ_RADIO_NONE, IRQ_RADIO_NONE);
        Radio.SetRx(TTtime_RX);//设置为接收模式
...
{
    //  Radio.SetDioIrqParams( TxIrqMask, TxIrqMask, IRQ_RADIO_NONE, IRQ_RADIO_NONE);
    SX1280HalWriteRegister( REG_LR_PAYLOADLENGTH,BufferSize );
    Radio.SendPayload( SendBuf, BufferSize, ( TickTime_t ) {
