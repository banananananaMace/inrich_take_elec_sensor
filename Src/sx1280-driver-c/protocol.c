...
uint32_t Message_cycle= 60*5,// 60000*3	,//10000  120000
...
float Alarm_threshold=150;//对应112℃
float Alarm_threshold_DOWN=-20;

extern uint32_t sensor_id ;
extern uint8_t Send_Frame_type; //配置参数过程 现在需要通信帧类型
extern uint8_t Send_REQ_time;
extern uint64_t default_sensor_id;
//6个字节头    send_Length +1个校验位
...
//电量数据发送
            ...
            CRC16_buf=RTU_CRC(&SndBuffer[2],send_Length+4);//8表示帧头＋ID共六个字节(-2表示除去CRC的两字节)
        ...
        if(sumtest==RecBuffer[2+6+(RecBuffer[1])])//校验和正确
        {
            ...
            case RSP_END :
                ...
                case communicate_type:
                    ...
                    if(Message_cycle > 60 * 60 ) //MESSAGE最大上限一小时发一次；
                    {
                        Message_cycle = 60 * 60 ;
                    }
                    if(Message_cycle < 90) //MESSAGE下限  90秒发一次；
                    {
                        Message_cycle = 90;
                    }
                    if(Ctrl_cycle < 12)
                ...
                case conrol_type:
                    Receive_CRC_HIGH = ((RecBuffer[RecBuffer[1]+7]<<8)+(RecBuffer[RecBuffer[1]+6]));
                    if (Receive_CRC_HIGH == CRC_Test)
                    {
                        ...
                        if((Alarm_threshold > 150)||(Alarm_threshold < 40))
                        {
                            Alarm_threshold = 150;
                        }
...
//                    {
//                    case Message_cycle_type :
//                        Message_cycle=((RecBuffer[12]<<24)+(RecBuffer[11]<<16)+(RecBuffer[10]<<8)+RecBuffer[9]);
//                        break;
//                    case Ctrl_cycle_type :

//                        Ctrl_cycle=((RecBuffer[10]<<8)+RecBuffer[9]);
//                        break;
//                    case Delay_type :
//                        Offset_Delay=((RecBuffer[12]<<24)+(RecBuffer[11]<<16)+(RecBuffer[10]<<8)+RecBuffer[9]);
//                        break;
//                    case Time_random_type://最大随机扰动时长
//                        Time_random=RecBuffer[9];
//                        break;
//                    case ALL_RSP_END_need_type :
//                        Message_cycle=((RecBuffer[12]<<24)+(RecBuffer[11]<<16)+(RecBuffer[10]<<8)+RecBuffer[9]);
//                        Ctrl_cycle=((RecBuffer[14]<<8)+RecBuffer[13]);
//                        Offset_Delay=((RecBuffer[18]<<24)+(RecBuffer[17]<<16)+(RecBuffer[16]<<8)+RecBuffer[15]);
//                        Time_random=(5*RecBuffer[19]);
//                        break;
//                    }//DATA_Type
//                }
...
//                    {
////                        Alarm_threshold=((RecBuffer[15]<<24)+(RecBuffer[14]<<16)+(RecBuffer[13]<<8)+RecBuffer[12]);
////                        Alarm_threshold_DOWN=((RecBuffer[19]<<24)+(RecBuffer[18]<<16)+(RecBuffer[17]<<8)+RecBuffer[16]);
//                        memcpy(&Alarm_threshold,&RecBuffer[12],4);
//                        memcpy(&Alarm_threshold_DOWN,&RecBuffer[16],4);
//                    }
//                }
...
//                        Ctrl_cycle=((RecBuffer[10]<<8)+RecBuffer[9]);
//                        Receive_RSP_End_OK_flag=1;//需要写入flash
//                        break;
//                    case Delay_type :
//                        Offset_Delay=((RecBuffer[12]<<24)+(RecBuffer[11]<<16)+(RecBuffer[10]<<8)+RecBuffer[9]);
//                        Receive_RSP_End_OK_flag=1;//需要写入flash
...
//                    case Time_random_type://最大随机扰动时长
//                        Time_random=RecBuffer[9];
//                        Receive_RSP_End_OK_flag=1;//需要写入flashTime_random
//                        break;
//                    case ALL_RSP_END_need_type :
//                        Message_cycle=((RecBuffer[12]<<24)+(RecBuffer[11]<<16)+(RecBuffer[10]<<8)+RecBuffer[9]);
//                        Ctrl_cycle=((RecBuffer[14]<<8)+RecBuffer[13]);
...
//                Ctrl_cycle = 12;
//            }
//            if((Alarm_threshold > 150)||(Alarm_threshold < 10))
//            {
//                Alarm_threshold = 150;
//            }
//            if((Alarm_threshold_DOWN < -40)||(Alarm_threshold_DOWN > 10))
